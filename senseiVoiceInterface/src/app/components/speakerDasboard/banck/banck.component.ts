...
import { FavoriteVoicesService, FavoriteVoicesDto } from '../../../services/favorite-voices.service';
...
@Component({
  selector: 'app-banck',
  templateUrl: './banck.component.html',
  styleUrls: ['./banck.component.css']
})
export class BanckComponent {
  languages = [
  ...
  getRandomHeight(index: number): number {
    // This creates a semi-random height for the audio wave bars
    // When not playing, it will be static
    if (this.currentPlayingVoiceId) {
      return Math.floor(Math.random() * 50) + 30; // Between 30% and 80%
    } else {
      // When not playing, create a static wave pattern
