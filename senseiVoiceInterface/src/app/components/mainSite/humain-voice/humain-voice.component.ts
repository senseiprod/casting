...
export class HumainVoiceComponent implements OnInit {
  ...
  mapLahajatiVoices(lahajatiVoices: any[]): LahajatiVoice[] {
    return lahajatiVoices.map((voice) => ({
      ...
      description:
        voice.description || `Voix Darija professionnelle ${voice.gender || ""} ${voice.dialect || ""}`.trim(),
      labels: {
        gender: voice.gender || "unknown",
  ...
  getVoiceRouteParams(voice: any): any[] {
    if (voice.language === "darija") {
      return [
        ...
        voice.labels?.gender || voice.gender || "Non défini",
        voice.labels?.age || voice.ageZone || "Non défini",
        voice.category || "professional",
        voice.fine_tuning?.language || voice.language || "darija",
