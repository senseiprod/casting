...
import  { FactureService, FactureSpeakerResponse } from "../../services/facture.service"
...
export class EarningComponent implements OnInit {
  ...
  calculateStatistics(): void {
    ...
    const totalAmount = this.transactions.filter((t) => !this.isWithdrawal(t)).reduce((sum, t) => sum + t.montant, 0)
    ...
    const monthsWithTransactions = new Set(transactionsThisYear.map((t) => new Date(t.dateTransaction).getMonth()))
  ...
  prepareTransactionDetails(): void {
    ...
    this.transactionDetails = this.transactions.map((transaction) => {
      ...
      const demande = this.demandes.find((d) => {
        ...
        return (d.type === "retrait" && d.code.includes(transaction.code)) || (facture && d.code.includes(facture.code))
      })
  ...
  private generateInvoicePDF(facture: any, selectedActions: ActionWithSelection[]): Promise<File> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header
        doc.setFontSize(20)
        doc.setTextColor(203, 16, 37) // Primary color
        doc.text("INVOICE", 20, 30)

        // Invoice details
        doc.setFontSize(12)
        doc.setTextColor(0, 0, 0)
        doc.text(`Invoice #: ${facture.code}`, 20, 50)
        doc.text(`Date: ${currentDate}`, 20, 60)
        doc.text(`Speaker ID: ${this.currentUserUuid}`, 20, 70)

        // Invoice title and description
        doc.setFontSize(14)
        doc.text("Invoice Details:", 20, 90)
        doc.setFontSize(12)
        doc.text(`Title: ${this.invoiceForm.get("invoiceTitle")?.value}`, 20, 105)

        // Description with text wrapping
        const description = this.invoiceForm.get("invoiceDescription")?.value
        ...
        doc.setFontSize(14)
        doc.text("Actions Included:", 20, tableStartY)

        // Prepare table data
        const tableData = selectedActions.map((action, index) => [
          (index + 1).toString(),
          action.code,
          action.text.substring(0, 50) + (action.text.length > 50 ? "..." : ""),
          this.formatDate(action.dateCreation.toString()),
          this.formatCurrency(action.amount),
        ])

        // Add table
        ;(doc as any).autoTable({
          startY: tableStartY + 10,
          head: [["#", "Code", "Description", "Date", "Amount"]],
          body: tableData,
          theme: "grid",
          headStyles: {
            fillColor: [203, 16, 37],
        ...
        doc.text(`Total Amount: ${this.formatCurrency(this.selectedActionsTotal)}`, 20, finalY)

        // Add signature if available
        if (this.signaturePreview) {
          doc.text("Signature:", 20, finalY + 20)
          // Add signature image
          doc.addImage(this.signaturePreview, "PNG", 20, finalY + 25, 60, 20)
        }
        ...
        doc.text(
          "This invoice was generated automatically by the Voice Generation System.",
          20,
        ...
        const pdfFile = new File([pdfBlob], `invoice-${facture.code}.pdf`, { type: "application/pdf" })
  ...
  submitWithdrawal(): void {
    ...
    this.factureService
      .ajouterFactureSpeaker(formData)
      .pipe(
        switchMap((facture) => {
          // 2. Generate PDF invoice
          return this.generateInvoicePDF(facture, selectedActions).then((pdfFile) => {
            // 3. Upload the PDF
            return this.factureService
              .uploadPdf(facture.uuid, pdfFile)
              .pipe(
                switchMap((uploadResponse) => {
                  console.log("PDF uploaded successfully:", uploadResponse)

                  // 4. Create a withdrawal request (demande)
                  ...
                  return this.demandeService.createDemande(withdrawalRequest).pipe(
                    switchMap((demande) => {
                      // 5. Create a withdrawal transaction
      ...
      .subscribe((response) => {
        if (response) {
          ...
          alert("Withdrawal request submitted successfully! PDF invoice has been generated and uploaded.")
